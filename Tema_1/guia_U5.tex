...
\begin{questions}
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[scale=0.8]{/home/shluna/UNAHUR/Física/Clases/figs/rugby.png}
        \caption{ }
        \label{fig:rugby}
    \end{figure}
    ...
    \begin{center}
        ...
        \fill[pattern=north east lines] ({5*cos(60)-0.5},{5*sin(60)}) rectangle ({5*cos(60)+0.5},{5*sin(60)+0.2});
        \draw[thick] ({5*cos(60)-0.5},{5*sin(60)}) -- ({5*cos(60)+0.5},{5*sin(60)});
        \draw[thick,-latex] (-0.5,0) -- (2,0) node[anchor=north]{$x$};
        \draw[thick,-latex] (0,-0.5) -- (0,2) node[anchor=east]{$y$};
        \draw[thick] (0,0) -- node[anchor=south east]{$L$} (60:5);
        \draw (0,1) arc (90:60:1);
        \node[anchor=south] at (75:1.1) {$\theta$};
        \draw ({5*cos(60)},{5*sin(60)-1}) arc (270:240:1);
        \node[anchor=north] at ({5*cos(60)-0.3},{5*sin(60)-1}) {$\theta$};
        \draw[dashed] (0,0) -- ({10*cos(60)},0);
        \draw[dashed] ({5*cos(60)},0) -- (60:5);
        \fill[black] (0,0) circle (0.5mm);
        \draw[dashed] ({5*cos(60)},0) ellipse (2.5 and 0.5);
        \draw[-latex] ({10*cos(60)},-0.35) arc [x radius = 2.7, y radius = 0.7, start angle = -30, end angle = 30];
        \node[anchor=west] at ({10*cos(60)+0.5},0) {$\omega$};
        \draw[thick,red,-latex] (0,0) -- (0,-1.5) node[anchor=west]{$\vec{F}_\text{g}$};
        \draw[thick,blue,-latex] (0,0) -- (60:2) node[anchor=north west]{$\vec{T}$};
        \end{tikzpicture}
    \end{center}

    \begin{parts}
        \part Observando la figura se pueden obtener las componentes de los vectores tensión $\vec{T}$ y peso $\vec{F}_\text{g}$:
        \begin{align*}
        \vec{T} &= \left(T \sen \theta ; T \cos \theta\right) \\
        \vec{F}_\text{g} &= \left(0; -m \, g\right)\\
        \end{align*} Estas son las únicas fuerzas que actúan sobre la pelota.
    
        Planteamos ahora las sumatorias de las componentes de las fuerzas en los ejes $x$ e $y$:
        \begin{align*}
        R_y &= T \cos \theta - m \, g = 0 \\
        R_x &= T \sen \theta = m \, a_\text{c}
    ...
    \begin{figure}[ht]
        ...
        \begin{subfigure}{0.45\textwidth}
            \centering
            \includegraphics[width=0.6\textwidth]{/home/shluna/UNAHUR/Física/Clases/figs/mesaydosmasas.jpg}
